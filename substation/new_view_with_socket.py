...
def send_data_to_server(line):
	...
	while True:
		...
		data= "68 04 07 00 00 00"	
		data1 ="68 04 43 00 00 00"
		data2 = "68 0E 00 00 00 00 64 01 06 02 01 00 00 00 00 14"

		hex_data=bytes.fromhex(data)
		print("hex_data---",hex_data)

		hex_data1=bytes.fromhex(data1)
		print("hex_data1---",hex_data1)

		hex_data2=bytes.fromhex(data2)
		print("hex_data2---",hex_data2)

		try:
			if(socket_con_flag == True):
				for k in range(2):
					print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkk---",k)

					seperate_data = send_command(hex_data)
					...
					seperate_data1 = send_command(hex_data1)
					for dt in seperate_data1[1:]:
						# final_dict = get_data(dt)
						get_data(dt)
						# print("final_dict2---+++++++----",final_dict)

						final_result.update(final_dict)


					seperate_data2 = send_command(hex_data2)

					for dt in seperate_data2[1:]:
						# final_dict = get_data(dt)
						get_data(dt)
						# print("final_dict3---+++++++----",final_dict)
						final_result.update(final_dict)
	...
	while True:
		...
		if(socket_con_flag == True):
			try:
				...
				seperate_data1 = recv_hex_data.split('68')
				# print("seperate_data ::::",seperate_data)
